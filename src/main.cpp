        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for(int i=0;i<sensor_fusion.size();i++)
			{
				...
				//speed of this car at particular instant in future
				check_car_s+=((double)prev_size*0.02*check_speed);

				if(car_lane==lane)
					ahead|=(check_car_s>car_s) && ((check_car_s-car_s)<30);
				else if(car_lane-lane==1) //car is in right lane
					right|=(check_car_s>car_s-30) && (check_car_s<car_s+30);
				else if(car_lane-lane==-1)  //car is in left lane
					left|=(check_car_s>car_s-30) && (check_car_s<car_s+30);
			}
			...
			else
			{
				if(lane!=1)
				{
					if((lane==0 && right==false) || (lane==2 && left==false))
						lane=1;
				}
			...
			//std::cout << "ptsx " << pts_x[0] << std::endl;
			tk::spline s;
			...
			{
				if(ref_velocity>max_vel)
					ref_velocity=max_vel;
				else if(ref_velocity<acc)
					ref_velocity=acc;
				...
				//rotation back to the global coordinates
